...
import (
	...
	authEx "github.com/containerops/dockyard/middleware/auth"
)
...
func checkOptions(options map[string]interface{}) (tokenAccessOptions, error) {
	...
	hostenabled, ok := options["hostenabled"]
	if ok {
		opts.hostenabled, _ = hostenabled.(int)
	}
...
func newAccessController(options map[string]interface{}) (auth.AccessController, error) {
	...
	return &accessController{
		...
		hostenabled: config.hostenabled,
		rootCerts:   rootPool,
		trustedKeys: trustedKeys,
	}, nil
}

func (ac *accessController) useHostForRealm(realm string, host string) string {
	protoIdx := strings.Index(realm, "//")
	if protoIdx > 0 {
		protoIdx += 2
	}
	protocol := realm[:protoIdx]
	if protoIdx+1 >= (len(realm) - 1) {
		return protocol + host
	}
	remain := realm[protoIdx+1:]
	pathIdx := strings.Index(remain, "/")
	return protocol + host + remain[pathIdx:]
}
...
func (ac *accessController) Authorized(ctx context.Context, accessItems ...auth.Access) (context.Context, error) {
	...
	*/
	req := authEx.Ctx.Req.Request
	realm := ac.realm
	if 1 == ac.hostenabled {
		//context.GetLogger(ctx).Debugf("hostenabled, will replace realm(%s) with host(%s)", realm, req.Host)
		realm = ac.useHostForRealm(realm, req.Host)
		//context.GetLogger(ctx).Debugf("hostenabled, the realm is changed to [%s]", realm)
	}
...
func init() {
	auth.Register("hwtoken", auth.InitFunc(newAccessController))
}
